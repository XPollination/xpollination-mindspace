...
### 2. Top-Level Nodes: The Mental Landscape

Thomas's mindmap has three top-level domains:

```
xpollination-mindspace (root)
├── HomeAssistant              — Infrastructure thinking
│   ├── Hetzner CX22 setup
│   ├── Synology DS218 services
│   ├── VPN network
│   ├── Claude Code setup
│   └── ...
├── xpollination-mcp-server    — Content pipeline thinking
│   ├── MCP server core
│   ├── Agentic development framework
│   └── ...
└── xpollination-mindspace     — Meta: thinking about the thinking tool itself
    ├── This vision document
    ├── PM tool evolution (v1→v2→v3→mindspace)
    └── ...
```

Each top-level node corresponds to a **GitHub repository**. The source of documentation is the repo itself. The mindmap is the index that connects them.

### 3. The Node Model: Intrinsic Properties
...
#### 3.2 Why DoR and DoD Are Intrinsic (Not External)

In traditional PM, "Definition of Ready" and "Definition of Done" are team-level checklists applied externally to all tasks. Thomas wants them to be **properties of the node itself**:

- Each node declares its own readiness criteria (what inputs it needs)
- Each node declares its own completion criteria (what outputs it must produce, what quality gates must pass)
- Any part of the tooling chain (orchestrator, agents, dashboard) can inspect the same fields to determine the node's state
- No external checklist to maintain separately — the node IS the checklist

This is the "object-oriented" principle: the node encapsulates its own lifecycle.
...
top_level_nodes:
  - id: "homeassistant"
    title: "HomeAssistant"
    repo:
      git_url: "https://github.com/PichlerThomas/HomeAssistant"
      local_path: "/home/developer/workspaces/github/PichlerThomas/HomeAssistant"
...
### 5. Graph Traversal and Node Rearrangement
...
#### 5.2 Node Rearrangement (Drag-and-Drop)
...
### 6. Root Cause Analysis Alignment
...
### 8. Evolution from v1 to Mindspace
...
#### 9.2 Data Model Migration (v2 YAML → Mindspace YAML)
...
### 11. Quality Gates for This PDSA
...
| QG-6 | Thomas approves vision as matching his intent | thomas |

**Self-assessment (QG-1 through QG-5):**
- QG-1: PASS — Section 3.1 defines `definition_of_ready` and `definition_of_done` as intrinsic node fields
- QG-2: PASS — Section 4 defines dual-link schema with concrete YAML examples
- QG-3: PASS — Section 5 documents traversal (scope stack, expand/collapse) and drag-and-drop (SortableJS)
- QG-4: PASS — Section 9 lists all files to migrate with source/target paths
- QG-5: PASS — Section 1 maps every PM concept to a mindmap concept; Section 8 traces the evolution

**QG-6: Pending Thomas review.**
